...
import { getTestForTaking, submitPlacementTest } from '../../services/api';
...
const renderMediaBlock = (block: SectionMedia, key: string | number): ReactNode => {
  ...
  if (block.type === 'audio') {
    return (
      <div key={`media-audio-${key}`} className="rounded-xl border border-purple-100 bg-purple-50/60 p-4 shadow-sm">
        <div className="text-xs font-semibold uppercase tracking-wide text-purple-700 mb-2">Audio</div>
        <audio
          controls
          ...
          sectionQuestions.map(({ question, globalIndex }) => {
            ...
            return (
              <div
                ...
                className={`rounded-2xl border bg-white shadow-sm transition ${isCurrent ? 'border-purple-400 ring-2 ring-purple-100 shadow-md' : 'border-slate-200 hover:border-purple-200/70'
                  }`}
              >
                <div className="flex flex-wrap items-start justify-between gap-3 border-b border-purple-100/70 bg-purple-50/60 px-5 py-4">
                    ...
                    <div className="flex flex-col gap-2">
                      {question.options?.map((option, optionIndex) => {
                        ...
                        const unselectedClass = allowMultiple
                          ...
                          : 'bg-white border-slate-200 hover:border-purple-200/70 hover:bg-purple-50/40 text-slate-700';
                  ...
                  {question.type === 'dropdown' ? (
                    <div className="space-y-2">
                      <select
                        ...
                        className="w-full p-3 border border-slate-300 rounded-lg bg-white text-slate-700 shadow-sm focus:border-purple-400 focus:ring-2 focus:ring-purple-400/60"
                      >
                  ...
                  {question.type === 'short_answer' ? (
                    <div>
                      <input
                        ...
                        className="w-full rounded-lg border border-slate-300 bg-white p-3 text-sm text-slate-700 shadow-sm focus:border-purple-400 focus:ring-2 focus:ring-purple-400/60"
                        type="text"
                      />
                    </div>
                  ) : null}
...
const TakeMockTestPage: React.FC = () => {
  ...
  const handleSubmit = useCallback(
    async (isAutoSubmit = false) => {
      ...
      try {
        ...
        const response = await submitPlacementTest(test._id, {
          answers: payload,
          durationSeconds: Math.max(0, (test.timeLimit ?? 0) * 60 - timeRemaining),
        });
        ...
        const resultId = response?.data?.resultId || response?.resultId;
        if (!resultId) {
          console.error('No resultId in response:', response);
          toast.error('Could not get result ID from server');
          setIsSubmitting(false);
          return;
        }

        console.log('Navigating to result page with ID:', resultId);
        navigate(`/mock-test/result/${resultId}`);
      } catch (error: unknown) {
    ...
    [answers, isSubmitting, navigate, test, timeRemaining]
  );
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200/60 bg-white/80 backdrop-blur-xl shadow-sm">
        <div className="flex w-full flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-12">
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-3 sm:gap-4">
            <div className="flex items-center gap-2 rounded-2xl border border-purple-200 bg-gradient-to-r from-purple-50 via-white to-purple-100 px-4 py-2 font-mono text-base font-semibold text-purple-600 shadow-inner">
...
export default TakeMockTestPage;