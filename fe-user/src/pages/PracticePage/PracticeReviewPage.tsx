...
const sanitizeSections = (sections: PracticeSection[] = []): PracticeSection[] => sections.map(s => ({
  ...s,
...
const PracticeReviewPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAttempt = async () => {
      ...
      setIsLoading(true);
      try {
        const res = await getPracticeAttemptDetail(attemptId);
        // @ts-ignore
        const attemptData = res.data.attempt;
        // @ts-ignore
        const practiceData = res.data.practice;

        setAttempt(attemptData);
        if (practiceData) {
           setPractice({ ...practiceData, sections: sanitizeSections(practiceData.sections) });
        }
      } catch (e) { console.error(e); toast.error('Error loading practice details.'); }
  ...
  const detailedResults = useMemo<DetailedResult[]>(() => {
    ...
    return practice.questions.map(q => {
      ...
      return {
        ...
        explanation: '' // Practice questions might not have explanation in the type yet, or it's not populated
      };
    });
  }, [practice, attempt]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-gray-600">Loading practice details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center p-8 bg-white rounded-2xl shadow-xl">
          ...
          <Button onClick={() => navigate('/practice')}>Back to Practice List</Button>
        </div>
      </div>
    );
  }
      ...
      <header className="shrink-0 z-40 border-b border-white/50 bg-white/80 backdrop-blur-md shadow-sm transition-all duration-300">
        <div className="flex w-full flex-wrap items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="sm" onClick={() => navigate(`/practice/attempts/${attemptId}`)} className="text-slate-500 hover:text-slate-900">
...
function QuestionResult({ question, detail, theme }: { question: PracticeQuestion, detail?: DetailedResult, theme: any }) {
