...
// Trang làm bài test IELTS duy trì để tương thích với chức năng cũ
const IELTSTestPage: React.FC<IELTSTestPageProps> = () => {
  ...
  // Tải đề thi và chuyển đổi thời gian sang giây để đếm ngược
  ...
  // Cập nhật câu trả lời cho từng câu dựa trên loại câu hỏi
  ...
  // Gom đáp án và gửi lên server, tránh gửi lặp khi đã submit
  ...
  // Định dạng thời gian đếm ngược thành chuỗi HH:MM:SS
  ...
  // Lấy section hiện tại để hiển thị passage/media
  ...
  const getCurrentQuestions = () => {
    ...
    // Gom nhóm câu hỏi theo phần hướng dẫn tương tự kiểu IELTS
    const section = getCurrentSection();
    if (!section) return [];
    
    const sectionQuestions = test.questions.filter(q => q.sectionId === section._id);
    
  // Nhóm câu hỏi theo từng block hướng dẫn
  ...
  // Render UI phù hợp cho từng loại câu hỏi
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header cố định hiển thị thông tin bài thi */}
  ...
  {/* Nội dung chính chia hai cột */}
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="grid grid-cols-2 gap-6 h-[calc(100vh-140px)]">
          {/* Cột trái: đoạn văn và tư liệu kèm theo */}
          ...
          <div className="bg-white rounded-lg shadow-sm border overflow-y-auto">
            <div className="p-6">
              ...
              {/* Điều hướng giữa các nhóm câu hỏi/section */}
              <div className="mt-8 flex justify-between items-center pt-6 border-t">
                ...
                <button
                  onClick={() => {
                    const maxGroups = Math.ceil(getCurrentQuestions().length / 10); // Tạm tính 10 câu mỗi nhóm
