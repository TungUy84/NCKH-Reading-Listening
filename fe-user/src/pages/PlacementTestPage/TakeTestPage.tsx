      ...
      <div className="space-y-4">
        {matchingPairs?.map((pair: any, i: number) => (
          <div key={i} className="space-y-2 p-3 rounded-xl bg-slate-50 border border-slate-100">
            ...
            <input 
              value={answer.matchingAnswers?.find((a: any) => a.prompt === pair.prompt)?.selected || ''}
              onChange={(e) => {
                const newPairs = matchingPairs.map((p: any) => ({
                  prompt: p.prompt,
                  selected: p.prompt === pair.prompt ? e.target.value : (answer.matchingAnswers?.find((a: any) => a.prompt === p.prompt)?.selected || '')
                }));
